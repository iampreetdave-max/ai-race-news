...
def _sitemap(public_dir: str, now: datetime, articles: list[dict] | None = None):
    pages = [
        ('/', '1.0', 'daily'),
    ...
    if articles:
        for a in articles[:200]:
            ...
            art_date = pub[:10] if pub and len(pub) >= 10 else lastmod
            lines += [
                '  <url>',
                f'    <loc>{SITE_URL}/article/{slug}</loc>',
                f'    <lastmod>{art_date}</lastmod>',
...
def export_all(db_path=None, output_dir=None):
    ...
    _sitemap(public_dir, now, all_articles)
